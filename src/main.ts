...
import {IEmitID, TEmitCustomerData} from './types';
...
events.on('catalog:activeItemSet', () => {
    ...
    if (card) {
        ...
        const buttonText = buttonIsDisabled ? 'Недоступно' : (isInBasket
                                                              ...
                                                              : 'Купить');
        modalView.render({
            content: new CardPreview(cloneTemplate<HTMLElement>('#card-preview'), events).render({
                ...card,
...
events.on('basket:update', () => {
    const cards = basketModel.getItems().map((item, index) => {
        return new BasketCardNew(cloneTemplate<HTMLElement>('#card-basket'), events).render({
            ...item,
            index: index + 1,
        })
    })
